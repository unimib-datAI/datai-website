...
- **Focal moment:** Site entry begins with a `1250ms` phosphor boot in the shared header: the literal `dat.ai` is written as CRT text, compressed into a horizontal beam, and replaced from that same beam by the untouched official Figma vector. On Home, the logo boot coordinates with the evidence scan: the thesis is revealed through a bounded raster-like clip, the rule draws left-to-right, and the three specimens develop in order while one acid scan line passes through each record. Neither sequence loops.
...
- `prefers-reduced-motion: reduce` shows the official logo immediately and removes the authored spatial sequence, panel wipe, histogram growth and specimen lift while preserving short color and border feedback.
